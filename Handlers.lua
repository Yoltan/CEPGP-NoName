...
function CEPGP_handleComms(event, arg1, arg2, response, lootGUID)
    ...
    -- print("CEPGP_handleComms", event, arg1, arg2, arg2 and CEPGP_NormalizeName(arg2), response, lootGUID, arg1 and string.lower(arg1)) --TODO
	...
	elseif event == "CHAT_MSG_WHISPER" and string.lower(arg1) == "!info" then
			...
			local name = arg2;
            name = CEPGP_NormalizeName(name);

		if CEPGP_getGuildInfo(name) ~= nil then
			local sender = name;
	...
	elseif event == "CHAT_MSG_WHISPER" and (string.lower(arg1) == "!infoguild" or string.lower(arg1) == "!inforaid" or string.lower(arg1) == "!infoclass") then
			...
			local nameFromArg2 = arg2;
            nameFromArg2 = CEPGP_NormalizeName(nameFromArg2);

		if CEPGP_Info.Guild.Roster[nameFromArg2] then
			local target = nameFromArg2;
...
function CEPGP_handleCombat(name)
	if (((C_PartyInfo.GetLootMethod() == 2 and CEPGP_isML() == 0) or (C_PartyInfo.GetLootMethod() == 3 and UnitIsGroupLeader("player"))) and CEPGP_ntgetn(CEPGP_Info.Guild.Roster) > 0) or CEPGP_Info.Debug then
		...
		local plurals = name == "The Four Horsemen" or name == "The Silithid Royalty" or name == "The Twin Emperors" or name == "Eredar Twins";
        local rescue = name == "Kalecgos";
		local message = format(L["%s " .. (plurals and "have" or "has") .. " been " .. (rescue and "rescued" or "defeated") .. "! %d EP has been awarded to the raid"], localName, EP);
