...
function CEPGP_distribute_popup_give()
	...
	for i = 1, 40 do
      ...
      --   print(
      --   "i:", i, 
      --   "x:", GetMasterLootCandidate(CEPGP_Info.Loot.SlotNum, i), --nil
      --   "a:", CEPGP_NormalizeName(GetMasterLootCandidate(CEPGP_Info.Loot.SlotNum, i)), --nil
      --   "b:", CEPGP_Info.DistTarget, --Hiderly-Thunderstrike
      --   "c:", CEPGP_Info.Loot.SlotNum --1
      -- )
      --TODO end
    ...
    if not candidateFound then
      CEPGP_print("The list of master loot candidates is empty. Attempting fallback...")
      for i = 1, GetNumGroupMembers() do
        local player = select(1, GetRaidRosterInfo(i));
        player = normalizeName(CEPGP_NormalizeName(player), true);
		if player == CEPGP_Info.DistTarget then
			if CheckInteractDistance(player, 2) then
				InitiateTrade(player)
			else
				CEPGP_print(player.." is too far away", true)
			end
			return;
		end
      end
...
function CEPGP_attendanceChange(self, arg1, arg2, checked)
	if (not checked) then
		LibDD:UIDropDownMenu_SetSelectedValue(CEPGP_attendance_dropdown, self.value)
